...
perfList <-  map(agentIDs, ~ getPerf(EMroot,. ))
...
pcList <-  map(agentIDs, ~ getDestProcConn(EMroot,. ))
...
pcList <-  map(agentIDs, ~ getSrcProcConn(EMroot,. ))
...
for (i in 1:length(pList)){
  if (nrow(pList[[i]]) > 0) {
    ...
    print(knitr::kable(head(pList[[i]][,c(3:8)], n = 10),  caption = RptHdr, col.names = c_names))
    cat("\n\n***")
  }
}
```
